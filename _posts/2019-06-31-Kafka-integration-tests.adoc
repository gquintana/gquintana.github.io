...
Such an integrations should be able to
...
To me this library lacks some syntactic sugar to make tests more readable.
...
Testcontainers is strongly integrated with JUnit 5, a single annotation and you're done. A JUnit 4 rule is also available.
[source,java]
----
...
<2> Create a Kafka container. 
	The https://hub.docker.com/r/confluentinc/cp-kafka[cp-kafka Docker image] created by Confluent is used. 
    As a consequence the version number matches the Confluent Platform version.
...
but process isolation is stronger, and you are closer to production.
...
It can take a long time for the consumer group controller to be elected, the partitions to be assigned. 
Between the consumer bootstrap and the first messages being received,
it can take a second or so.

Using a `ConsumerRebalanceListener` to check which and when partitions are assigned can be useful.
