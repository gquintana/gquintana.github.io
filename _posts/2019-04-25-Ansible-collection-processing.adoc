...
In this article, I'll show how to process a list of users:
[source,yaml]
----
...
Since Ansible 2.7, the `map` filter can take 3 arguments: the attribute, an operator and arguments.
...
What we did with the `users` variable can also be done, with variables provided by Ansible:
----
include::{source_dir}/group_vars/all.yml[tags=host_var]
----
<1> Take all hosts from group `all`, then grab the `ansible_hostname` host variable for each of them and join them.
...
In my humble opinion, being able to call a Python function would make thinks easier.
