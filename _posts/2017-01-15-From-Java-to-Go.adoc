...
It has been translated in many languages. In a word, awesome!
...
* I can not easily chain function calls
  ...
  I would have to creates a function, but with the multiple type `*Article, error`, but it's not possible.
...
This is a rather common scenario when you are refactoring or trying to implement an idea.
