...
:image_dir: ../images/2017-01-15-From-Java-to-Go
...
You can event run the web server locally if you want to learn Go while traveling,
...
Go doesn't allow dynamic linking, this forces Go libraries to be open-source.
...
First of all, a *channel* is a concept used for inter-thread communications.
...
go doit()
----
The `doit` function is ran in background

image::{image_dir}/concurrency.svg[title=Channels & Goroutines,role=right]
[source,go]
----
...
----
<1> I wonder why the `NewPerson` constructor and the `GetName` function are not declared inside the struct.
...
<2> Function should never take a pointer to an interface,
...
<3> The variable `p` of type `*Person` is automatically converted into `Nameable` interface.
    Go is doing duck typing, if it has a `GetName` person, then it is a `Nameable` thing.
