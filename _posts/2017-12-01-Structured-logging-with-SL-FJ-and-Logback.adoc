...
How can we write this data structure on disk or send it over the wire to log collection tool tool like Elasticsearch?
We must serialize it,
however we should not flatten it in dumb text format and loose the structure.
We must keep it in a structured format like JSON.

A structured format like JSON also can also handle properly multiline logs like stack traces/call traces or messages containing line separators (wanted or not).
