...
One of my 2017 resolutions wille be to try different languages and see how they compare to Java.
...
I've seen and used Ruby many times: Vagrant configuration, Logstash plugins, FluentD plugins, Rails applications...
...
The second one shows the templating system in strings:
[source,ruby]
----
i=4
...
There is a an `unless` keyword which does the contrary of `if`
[source,ruby]
----
...
puts 1.even?         # <3>
----
<1> Let's add method to integers
...
Ruby libraries and applications are packaged as _Gems_ which are Zip archive
along with metadata like Java's Jar files.
...
I had to read each tool documentation, look for an example and assemble the tools.
